    ...
    <div class="w-[80%] m-auto">
      <div class="min-h-96" v-loading="loading">
        <Card class="mb-4" size="small  " v-for="(item, index) in configList" :key="index">
          ...
          <SwitchSetting
              v-if="!['SYS_WATERMARK_SWITCH'].includes(item.configSn) && item.configSn.endsWith(END_WITH_SWITCH)"
              :data-item="item"
          />
          ...
          />
          <WorkbenchLayoutSetting
              v-if="['SYS_COM_WORKBENCH_LAYOUT'].includes(item.configSn)"
              :data-item="item"
          />
          <div v-else>
            {{item.configSn}}
...
import WorkbenchLayoutSetting from "./components/WorkbenchLayoutSetting.vue";
