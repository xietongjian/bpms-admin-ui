<template>
  <Page auto-content-height>
      ...
      <template #nameContent="{ row }">
        <div class="bg-secondary m-2 p-2">
          <Descriptions v-if="row.type === 'sc'" :title="null" size="small">
            <DescriptionsItem label="请求地址" :span="3">{{
              row.url || '-'
            }}</DescriptionsItem>
            <DescriptionsItem label="请求方式">{{
              row.method || '-'
            }}</DescriptionsItem>
            <DescriptionsItem label="Token名称">{{
              row.tokenName || '-'
            }}</DescriptionsItem>
            <DescriptionsItem label="Token键名">{{
              row.tokenKey || '-'
            }}</DescriptionsItem>
            ...
            <DescriptionsItem label="请求参数" :span="3">
              <pre>{{
                JSON.stringify(JSON.parse(row.params), null, 2) || '{}'
              }}</pre>
            </DescriptionsItem>
    ...
    <BizTokenConfigDrawer
      ref="bizTokenConfigDrawerRef"
