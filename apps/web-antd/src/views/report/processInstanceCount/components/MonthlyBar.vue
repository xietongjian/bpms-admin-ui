<script setup lang="ts">
  ...
  const { setOptions, getInstance, echarts } = useECharts(chartContent as Ref<HTMLDivElement>);
  ...
  async function fetchData() {
    try {
      ...
      const date = (month.value = presetDate?.slice(0, 10));

      if (date) {
        showDaysChart.value = false;

        const requestMethod =
          props.requester === 'task' ? getTaskCountsForOneMonth : getInstanceCountsForOneMonth;
