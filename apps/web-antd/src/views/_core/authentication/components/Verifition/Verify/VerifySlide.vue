<template>
  <div style="position: relative">
    ...
    >
      ...
      >
        <img
          :src="backImgBase ? backImgBase : defaultImg"
...
import { getVerifyCaptcha, checkVerifyCaptcha } from '#/api/core/auth'
import defaultImgUrl from '../image/default.png'
...
const { vSpace, imgSize, barSize, blockSize, explain } = toRefs(props)
...
function start(e) {
  ...
  if (!isEnd.value) {
    text.value = ''
    moveBlockBackgroundColor.value = '#166AFF'
    leftBarBorderColor.value = '#166AFF'
    iconColor.value = '#fff'
    e.stopPropagation()
    status.value = true
  }
...
function end() {
  ...
  if (status.value && isEnd.value == false) {
    ...
    let data = {
      position: moveLeftDistance,
      captchaId: backToken.value,
    }
    checkVerifyCaptcha(data).then((res) => {
      if (res.success) {
        props.loginFn(data)
...
function getPictrue() {
  getVerifyCaptcha().then((res) => {
    backImgBase.value = res.bgImage
    blockBackImgBase.value = res.sliderImage
    backToken.value = res.id
    top.value = res.gapY
  })
}
...
.arrow-right:before {
  ...
  bottom: 0;
  z-index: 9999;
  background: url(../image/arrow-right.png) no-repeat 0 0 / contain;
}
