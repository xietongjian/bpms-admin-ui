...
import {flowFrameworkFormSchema, flowSystemFormSchema, formSchemaBusiness} from './framework.data';
import {Badge, Button, Col, message, Row, Space, Steps, Tooltip, TypographyText} from 'ant-design-vue';
...
async function loadBaseInfo(id, type, pid) {
  ...
  if (type === 'framework') {
    await formApi.updateSchema(flowFrameworkFormSchema);
  } else {
    await formApi.updateSchema(flowSystemFormSchema);
  }
  ...
  if (id) {
    ...
    if (type === 'framework') {
      ...
      await formApi.setValues({pid: pid || ''});
    }
  }
...
const [BasicModal, modalApi] = useVbenModal({
  ...
  async onOpenChange(isOpen: boolean) {
    if (isOpen) {
      ...
      if (values) {
        dataType.value = values.type;
        loadBaseInfo(values.id, values.type, values.pid);
      }
    }
  },
  onConfirm() {
