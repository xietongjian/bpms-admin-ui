<template>
  <div>
    <Space>
      <!-- 撤回功能逻辑 -->
      <Popconfirm v-if="revokeVisible" :title="`确定要撤回流程吗？`" @confirm="handleRevoke" type="error" >
        {{ revokeAuthPointer.name }}
      </Popconfirm>

      <Button v-if="fetchBackVisible" @click="handleFetchBack" type="primary">取回</Button>
...
<script lang="ts" setup>
  import { defineEmits, defineProps, defineExpose, ref, unref, onMounted, nextTick } from 'vue';
  ...
  const emit = defineEmits(['refreshApproveHistory'])
