...
import { systemAiTypeSearchFormSchema, externalAiTypeColumns, externalAiTypeFormSchema } from './externalAiType.data';
import { pageListFlowNodeType, deleteFlowNodeType, saveOrUpdateFlowNodeType } from '#/api/flow/externalAi';
import { useVbenForm } from '@vben/common-ui';

const [Form, formApi] = useVbenForm({ commonConfig: { labelWidth: 100 }, schema: externalAiTypeFormSchema });
...
const [Modal, modalApi] = useVbenModal({
  ...
  onConfirm: async () => {
    ...
    try {
      const { success, msg } = await saveOrUpdateFlowNodeType(values);
...
const formOptions: VbenFormProps = {
  ...
  schema: systemAiTypeSearchFormSchema,
};

let reloadGrid: () => void;
const gridOptions: VxeGridProps = {
  columns: externalAiTypeColumns, columnConfig: { resizable: true }, height: 'auto',
  keepSource: true, border: false, stripe: true, showOverflow: false,
  proxyConfig: { ajax: { query: async ({ page }, formValues) => await pageListFlowNodeType({ ...formValues, pageSize: page.pageSize, pageNum: page.currentPage }) } },
};
...
async function handleDelete(record: any) { await deleteFlowNodeType({ id: record.id }); await reload(); }
</script>

<template>
  <Page auto-content-height>
    <BasicTable table-title="AI节点类型" @create="handleCreate">
      <template #action="{ row }">
