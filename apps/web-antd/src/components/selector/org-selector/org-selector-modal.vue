<template>
  <BasicModal
    class="min-h-[400px] w-[800px] overflow-y-hidden"
...
function multiFilterTree(tree, name, isOpen) {
  ...
  for (const node of tree) {
    const isNameMatch =
      name === null || (typeof name === 'string' && node.name.includes(name));
...
watch(searchValue, (value) => {
  // search();
  const res = filterArrForKey({
    tree: treeListData.value,
    searchKey: 'name',
...
function handleCheck(e, record, b) {
  ...
  if (!multiSelect.value) {
    selectedRowKeys = JSON.parse(
      JSON.stringify(e.checked.length > 0 ? [e.checked[0]] : []),
    );
  } else {
